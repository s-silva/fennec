    ...
    case CONV(2,0):
        if (hDecoder->upMatrix)
        {
            ...
            for(i = 0; i < frame_len; i++)
            {
				...
				inp0 *= FLOAT_SCALE;
				CLIP(inp0, 1.0, -1.0f);
                (*sample_buffer)[(i*2)+0] = (double)inp0;
                (*sample_buffer)[(i*2)+1] = (double)inp0;
            }
        } else {
            ch  = hDecoder->internal_channel[0];
            ch1 = hDecoder->internal_channel[1];
            for(i = 0; i < frame_len; i++)
            {
                ...
                (*sample_buffer)[(i*2)+0] = (double)inp0;
                (*sample_buffer)[(i*2)+1] = (double)inp1;
            }
        }
        break;
    default:
        for (ch = 0; ch < channels; ch++)
        {
            for(i = 0; i < frame_len; i++)
            {
				...
				inp *= FLOAT_SCALE;
				CLIP(inp, 1.0, -1.0f);
				(*sample_buffer)[(i*channels)+ch] = (double)inp;
            }
        }
        break;
    }
