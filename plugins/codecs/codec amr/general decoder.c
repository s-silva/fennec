...
string info_base = uni("AMR Codec");
...
{
	MessageBox((HWND)pdata, uni("Adaptive Multi-Rate Codec (AMR)."), uni("About AMR Decoder"), MB_ICONINFORMATION);
	return 1;
}
...
{
	MessageBox((HWND)pdata, uni("Sorry, No settings."), uni("AMR Decoder Settings"), MB_ICONINFORMATION);
	return 1;
}
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("amr"));
		str_cpy(out_desc, uni("Adaptive Multi-Rate Codec File"));
		break;
