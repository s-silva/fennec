...
struct plugin_stream
{
	...
	int               sel_stream_video;
	int               sel_stream_audio;
	AVCodecContext   *avc_audio_context;
	AVCodec          *avc_codec_audio;
	AVCodecContext   *avc_video_context;
	AVCodec          *avc_codec_video;
	AVStream         *video_stream;
	AVStream         *audio_stream;
	DECLARE_ALIGNED(16, uint8_t, obuffer[(AVCODEC_MAX_AUDIO_FRAME_SIZE * 3) / 2]);
	int               ob_start, ob_size;

	uint8_t          *vid_audio_based_last_frame;
	int               newrun;
	int               last_w, last_h;
	int               frame_got_new;

	struct queue_frame  vid_queue;
