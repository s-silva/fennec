...
string info_base = uni("Monkey's Audio Codec");
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("Monkey's Audio Codec for Fennec Player."), uni("About Monkey's Audio Codec"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("Monkey's Audio Codec Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("ape"));
		str_cpy(out_desc, uni("Monkey's Audio"));
		break;
