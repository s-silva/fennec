...
string encoder_info_base = uni("Monkey's Audio Codec");
...
#	if defined(system_microsoft_windows)
		...
		MessageBox((HWND)odata, uni("Monkey's Audio Codec for Fennec Player."), uni("About Monkey's Audio Codec"), MB_ICONINFORMATION);
...
int callc encoder_get_initialization_info(unsigned long inforreq, void* outinfo, unsigned long* outsize)
{
	...
	{
		str_ncpy((string)outinfo, encoder_info_base, min(*outsize + 1, str_len(encoder_info_base) + 1));
