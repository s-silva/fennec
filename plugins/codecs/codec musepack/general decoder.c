...
string info_base = uni("Musepack Codec");
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("Musepack Codec."), uni("About Musepack Codec"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("Musepack Codec Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("mpc"));
		str_cpy(out_desc, uni("Musepack Files"));
		break;
