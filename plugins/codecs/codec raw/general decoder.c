...
string info_base = uni("Raw Audio Codec");
...
int callc fennec_plugin_getformat(unsigned long id, unsigned long* ftype, void* fdata)
{
	...
	case fennec_input_format_base + 1: /* from user selected */
		pstreams[id].filebps       = ((struct base_format*)fdata)->bbits;      
		pstreams[id].frequency     = ((struct base_format*)fdata)->bfrequency; 
		pstreams[id].channels      = ((struct base_format*)fdata)->bchannels;  
		break;
...
int callc fennec_plugin_getduration(unsigned long id, unsigned long* ptype, void* pos)
{
	...
	{
	case fennec_input_position_32ms:
		if(pstreams[id].frequency && pstreams[id].channels && pstreams[id].filebps && pstreams[id].filesize)
			pstreams[id].duration = (unsigned long)pstreams[id].filesize / (pstreams[id].filebps / 8) / (pstreams[id].frequency / 1000) / pstreams[id].channels;
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("Raw Audio Codec for Fennec Player."), uni("About Raw Audio Codec"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("Raw Audio Codec Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("raw"));
		str_cpy(out_desc, uni("Raw Audio File"));
		break;
