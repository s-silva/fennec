...
string info_base = uni("Speex Codec");
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("Speex Codec for Fennec Player."), uni("About Speex Codec"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("Speex Codec Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("spx"));
		str_cpy(out_desc, uni("Speex Codec"));
		break;
