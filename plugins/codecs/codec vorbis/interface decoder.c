...
string info_base = uni("Vorbis Decoder");
...
unsigned long callc fennec_plugin_open(unsigned long otype, unsigned long osize, const string odata)
{
	...
	if(otype == fennec_input_open_file || otype == fennec_input_open_internet_stream)
	{
		...
		if(!decoder_load(j, odata))
		{
			pstreams[j].initialized = 0;
			j = 0;
		}
...
{
	MessageBox((HWND)pdata, uni("Ogg Vorbis codec for Fennec 7.1 (Player 1.0)\nhttp://www.xiph.org/"), uni("About Ogg Vorbis Decoder"), MB_ICONINFORMATION);
	return 1;
}
...
{
	MessageBox((HWND)pdata, uni("Sorry, No settings."), uni("Ogg Vorbis Decoder Settings"), MB_ICONINFORMATION);
	return 1;
}
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	{
	case 0:
		str_cpy(out_ext,  uni("ogg"));
		str_cpy(out_desc, uni("Ogg Audio File"));
		break;
	}
	return 1;
}
...
{
	return write_ogg_tag(fname, rtag);
}
