...
string encoder_info_base = uni("Vorbis Encoder");
...
int callc encoder_fennec_plugin_uninitialize(void)
{
	...
	pestreams_count = 0;
	if(pestreams)sys_mem_free(pestreams);
	pestreams = 0;
	return 1;
}
...
int callc encoder_about(unsigned long id, void* odata)
{
	...
	MessageBox((HWND)odata, uni("Ogg Vorbis codec for Fennec 7.1 (Player 1.0)\nhttp://www.xiph.org/"), uni("About Ogg Vorbis Codec"), MB_ICONINFORMATION);
...
int callc settings_fileencoder(unsigned long id, void* odata)
{
	...
	{
		DialogBox(hInst, (LPCTSTR) MAKEINTRESOURCE(IDD_SETTINGS), (HWND)odata, (DLGPROC) proc_settings_fileencoder);
	}
	return 1;
}
...
int callc encoder_get_initialization_info(unsigned long inforreq, void* outinfo, unsigned long* outsize)
{
	...
	{
		str_ncpy(outinfo, encoder_info_base, min(*outsize + 1, str_size(encoder_info_base) + 1));
		*outsize = (unsigned long)str_size(encoder_info_base);
		return 1;
	}
	return 0;
}
