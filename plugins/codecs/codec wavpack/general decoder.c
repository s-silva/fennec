...
string info_base = uni("WAVPACK Codec");
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("WAVPACK Codec for Fennec Player.\nCopyright (c) 1998 - 2007 Conifer Software. All rights reserved."), uni("About WAVPACK Codec"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("WAVPACK Codec Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("wv"));
		str_cpy(out_desc, uni("WAVPACK Codec"));
		break;
