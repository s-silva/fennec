...
string info_base = uni("AC-3 Decoder");
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("AC-3 Decoder for Fennec Player."), uni("About AC-3 Decoder"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("AC-3 Decoder Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("ac3"));
		str_cpy(out_desc, uni("AC-3 Files"));
		break;
