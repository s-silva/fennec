...
string info_base = uni("Line In Decoder");
...
int callc fennec_plugin_read(unsigned long id, unsigned long dsize, unsigned long* dread, void* rdata)
{
	...
	*dread = decoder_read(id, (void*)rdata, dsize);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("Line-in Decoder for Fennec Player."), uni("About Line-in Decoder"), MB_ICONINFORMATION);
...
#	if defined(system_microsoft_windows)
	...
	MessageBox((HWND)pdata, uni("There's nothing to configure."), uni("Line-in Decoder Settings"), MB_ICONINFORMATION);
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	case 0:
		str_cpy(out_ext,  uni("linein"));
		str_cpy(out_desc, uni("Line-in Decoder"));
		break;
