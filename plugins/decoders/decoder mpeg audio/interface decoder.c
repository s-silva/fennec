...
string info_base = uni("MPEG Audio Decoder");
...
int callc fennec_plugin_getformat(unsigned long id, unsigned long* ftype, void* fdata)
{
	...
	case fennec_input_format_base:
		((struct base_format*)fdata)->bbits      = pstreams[id].decdata.bits;
		((struct base_format*)fdata)->bfrequency = pstreams[id].decdata.frequency;
		((struct base_format*)fdata)->bchannels  = pstreams[id].decdata.channels;
		break;
...
{
	return decoder_score(fname, foptions);
}
...
int callc fennec_plugin_getduration(unsigned long id, unsigned long* ptype, void* pos)
{
	...
	case fennec_input_position_32ms:
		...
		*((unsigned long*)pos) = decoder_getduration_ms(id);
		break;

	case fennec_input_position_seconds:

		*((unsigned long*)pos) = decoder_getduration_ms(id) / 1000;
		break;
...
int callc fennec_plugin_read(unsigned long id, unsigned long dsize, unsigned long* dread, void* rdata)
{
	...
	if(!decoder_readdata(id, rdata, &rsize) || (rsize < dsize))
	{
		*dread = rsize;
...
{
	MessageBox((HWND)pdata, uni("MPEG Audio Decoder for Fennec 7.1 (Player 1.0) (Uses libmad, copyright (C) 2000-2004 Underbit Technologies, Inc.)"), uni("About MPEG Audio Decoder"), MB_ICONINFORMATION);
	return 1;
}
...
{
	MessageBox((HWND)pdata, uni("Sorry, No settings."), uni("MPEG Audio Decoder Settings"), MB_ICONINFORMATION);
	return 1;
}
...
int callc fennec_plugin_get_initialization_info(unsigned long inforreq, void* outinfo, unsigned long* outsize)
{
	...
	if(inforreq == fennec_information_basic)
	{
		...
		*outsize = (unsigned long)str_len(info_base);
		return 0;
	}
	return 0;
}
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	{
	case 0:
		str_cpy(out_ext,  uni("mp1"));
		break;
	case 1:
		str_cpy(out_ext,  uni("mp2"));
		break;
	case 2:
		str_cpy(out_ext,  uni("mp3"));
		break;
	}

	str_cpy(out_desc, uni("MPEG audio files"));
	return 1;
}
