...
string info_base = uni("Redbook Audio (Audio CD) Decoder");
...
int callc fennec_plugin_about(void* pdata)
{
	...
	MessageBox((HWND)pdata, uni("Audio CD decoder plug-in for Fennec 7.1.1 Player 1.1 (Windows 9x, ME - Not supported)."), uni("About Audio CD Decoder"), MB_ICONINFORMATION);
...
{
#ifdef system_microsoft_windows
	MessageBox((HWND)pdata, uni("Sorry, No settings."), uni("Audio CD Decoder Settings"), MB_ICONINFORMATION);
#endif
	return 1;
}
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	{
	case 0:
		str_cpy(out_ext,  uni("cda"));
		str_cpy(out_desc, uni("Redbook audio"));
		break;
	}
	return 1;
}
