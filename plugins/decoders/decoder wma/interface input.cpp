...
string info_base = uni("Windows Media Decoder");
...
{
	MessageBox((HWND)pdata, uni("Windows Media Audio decoder version 1.0 for Fennec 7.1 Player 1.0"), uni("About Windows Media Decoder"), MB_ICONINFORMATION);
	return 1;
}
...
{
	MessageBox((HWND)pdata, uni("Sorry, No settings."), uni("Windows Media Decoder Settings"), MB_ICONINFORMATION);
	return 1;
}
...
int callc fennec_plugin_get_extension(int id, string out_ext, string out_desc, int* newdsc)
{
	...
	{
	case 0:
		str_cpy(out_ext,  uni("wma"));
		str_cpy(out_desc, uni("Windows Media"));
		break;
	}
	return 1;
}
