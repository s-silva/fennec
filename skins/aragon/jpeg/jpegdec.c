...
void precalculate_Cr_Cb_tables()
{
 ...
 for (Cr_v=0;Cr_v<=255;Cr_v++)
  for (Cb_v=0;Cb_v<=255;Cb_v++)
   Cr_Cb_green_tab[((WORD)(Cr_v)<<8)+Cb_v]=(SWORD)(int)(-0.34414*(Cb_v-128.0)-0.71414*(Cr_v-128.0));
...
// and 1x1,1x1,1x1 = no upsampling needed
 ...
 Hmax=YH,Vmax=YV;
 if ( *X_image%(Hmax*8)==0) X_round=(WORD)*X_image; // X_round = Multiple of Hmax*8
 else X_round=(WORD)(*X_image/(Hmax*8)+1)*(Hmax*8);
 if ( *Y_image%(Vmax*8)==0) Y_round=(WORD)*Y_image; // Y_round = Multiple of Vmax*8
 else Y_round=(WORD)(*Y_image/(Vmax*8)+1)*(Vmax*8);
